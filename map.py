...
Date: 2024-03-24 10:46:52
LastEditors: Suizhi HUANG && sunrisen.huang@gmail.com
LastEditTime: 2024-05-08 21:55:01
FilePath: /HPV_test/map.py
Description: 
...
sparse_feature_list = [
    ...
    "被动吸烟（1:每周三次以上， 2:每周五次以上 ，3:少于一周一次）",
    ...
    "避孕药标志（否-0；是-1）",
    "口服避孕药类型（1:不详； 2:雌孕激素联合短效避孕药； 3.单孕激素短效避孕药； 4.紧急避孕药； 5.长效避孕药）",
    "避孕药最长使用年限（半年以内-1；一年以内-2；两年以内-3；三年以内-4；三年以上-5）",
    "HRT治疗标志（无-0；有-1）",
    "使用节育器标志（否-0；是-1）",
    "节育器使用时间（一年以内-1；三年以内-2；五年以内-3；十年以内-4；十年以上-5）",
    "性伴侣个数(1-1;2-2;>=3-3)",
    "月经间期出血（否-0；是-1）",
    "阴道不适（否-0；是-1）",
    "性交痛（否-0；是-1）",
    ...
    "目前筛查频率（五年一次-1；三年一次-2；两年一次-3；一年一次-4；半年一次-5；其他-6）",
    "细胞学检查次数（一次-1；两次以上-2；五次以上-3；不详-4）",
...
def convert(file_name, sparse_feature_list, dense_feature_list):
    ...
    # df.dropna(
    #     inplace=True,
    #     subset=[f'C{idx}' for idx in range(1, i)] + [f'I{idx}' for idx in range(1, j)],
    # )
    df = df.sample(frac=1)
...
if __name__ == '__main__':
    convert("new_data.csv", sparse_feature_list, dense_feature_list)
